...
Note that, following similar guidelines to previous CVPR conferences, the rebuttal is meant to address only the reviewers' comments and questions. It is not intended to add new material (theorems, algorithms, experiments) which was not included in the original submission. You may optionally add a figure, graph or proof to your rebuttal, but only to better illustrate your answer to the reviewers' comments. These additions must be minimal (as they are meant to address only the reviewers' comments). You must not use the rebuttal to add any new results or contributions.
