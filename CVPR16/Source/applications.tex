...
\subsubsection{Inference on graphical model}
We apply Metropolis Hastings' method~\cite{mackay1998introduction} for performing inference on graphical model. Since we are
optimizing over continuous variables, we use Gaussian distribution as
the proposal function. We choose Metropolis Hastings over
Blocked Coordinate Descent which turns out to be slower than Metropolis Hastings
