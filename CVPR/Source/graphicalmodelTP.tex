...
\begin{tikzpicture}[grow cyclic, line width=1.2,
    ...
    bboxfactor/.style= {rectangle,inner sep=2,fill=green,thick,text=green},
    level distance=30,
...
\draw (fdet)
--
++(90:1) node[variablenode,obs](Det){2} +(-.85,0) node {$\bb{2}$};
  
\draw (fpt) 
++(-90:1) node[variablenode,obs](pt){2} +(-.85,0) node {$\trackpit{2}{t}$} 
  ;
...
\coordinate (oneR) at (1.5,0);
  ...
  ++(oneR) node [anchor=west] (e1e) {$\Energy{size}$}
  ++(oneR) node [] (e2s) {}
  ++(oneR) node [anchor=west] (e2e) {$\Energy{dyn}$}
  ++(oneR) node [] (e3s) {}
  ++(oneR) node [anchor=west] (e3e) {$\Energy{lane}$};
\path (-4.8, -3.5)
  ++(oneR) node [] (e4s) {}
  ++(oneR) node [anchor=west] (e4e) {$\pEnergy{track}$}
 ++(oneR) node [] (e5s) {}
  ++(oneR) node [anchor=west] (e5e) {$\pEnergy{bbox}$}
  ;
