\begin{tikzpicture}[x=0.8cm,y=0.8cm,3d/perspective eye={4,4,-10},ptstyle/.style={fill=#1,draw,circle,inner sep=1.5pt}]
    ...
    \draw [->, very thick, draw=blue] (pta4) --  +(90:1) node [anchor=south,align=center] {$a^1(\lambda) = 0$\\$a^2(\lambda) = 1$};
    \draw [->, very thick, draw=red!50!blue] (pta-8) --  +(170:1.4) node [anchor=south,align=center] {$a^1(\lambda) = 0.5$\\$a^2(\lambda) = 0.5$};
    \draw [->, very thick, draw=red] ($(ptb-8)+(.5,-.8)$) --  +(-60:.8) node [anchor=north,align=center] {$a^1(\lambda) = 1$\\$a^2(\lambda) = 0$};
