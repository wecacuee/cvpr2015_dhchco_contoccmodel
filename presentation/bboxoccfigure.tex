...
\begin{tikzpicture}[3d/perspective eye={4,4,-10}]
  ...
  \end{scope}
   ...
   \node [anchor=north west] at (gdpb) {B};
   \node [anchor=south west] at (gb) {C};
   \node [anchor=south east] at (gb -| gh) {D};
   \node [anchor=south] at (C) {E};
   \fill [red!30] (gh) -- (E) -- (b) -- (F) -- cycle;
   \node [anchor=west] at ($(C) + (1,0)$) {$v_{AB} = \frac{\text{area(\tikz \node [rectangle,inner sep=0,fill=red!30,text=red!30] {0};)}}{\text{area}(\bigtriangleup ABE)}$};
\end{tikzpicture}
